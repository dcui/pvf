...
static int mana_pvf_register_hw_vport(struct mana_port_context *apc)
{
	...
	req.attached_gfid = 1;
	req.is_pvf_default_vport = 1;
	req.allow_vlan_tagging = 1;
	req.allow_all_ether_types = 1;
	req.allow_src_mac_spoofing = 1;
	req.mask_vlan_tag = 0;
	req.strip_vlan_tag = 0;
...
static void mana_pvf_deregister_hw_vport(struct mana_port_context *apc)
{
...
static int mana_pvf_register_filter(struct mana_port_context *apc)
{
	...
	req.allow_any_vlan_tag = 0;
	req.match_inner_mac_tni = 0;
	req.requirev_vlan_tag = 0;
	req.vlan = 0;
	req.tni = 0;
	...
	apc->pvf_filter_handle = resp.filter_handle;
	return 0;
}

static void mana_pvf_deregister_filter(struct mana_port_context *apc)
{
	...
	mana_gd_init_req_hdr(&req.hdr, MANA_DEREGISTER_FILTER,
			     sizeof(req), sizeof(resp));
	req.filter_handle = apc->pvf_filter_handle;
...
static void mana_destroy_vport(struct mana_port_context *apc)
{
	...
	if (gd->gdma_context->is_pvf)
		mana_pvf_deregister_hw_vport(apc);
}
			     ...
			     struct net_device *net)
{
	...
	if (gd->gdma_context->is_pvf) {
		err = mana_pvf_register_hw_vport(apc);
		if (err)
			return err;
	}
...
int mana_alloc_queues(struct net_device *ndev)
{
	...
	if (gd->gdma_context->is_pvf) {
		err = mana_pvf_register_filter(apc);
		if (err)
			goto destroy_vport;
	}
...
static int mana_dealloc_queues(struct net_device *ndev)
{
	...
	if (gd->gdma_context->is_pvf)
		 mana_pvf_deregister_filter(apc);
			   ...
			   struct net_device **ndev_storage)
{
	...
	*ndev_storage = ndev;
	...
	apc->pvf_filter_handle = INVALID_MANA_HANDLE;
