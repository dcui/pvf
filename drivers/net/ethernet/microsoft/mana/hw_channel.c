					...
					struct gdma_event *event)
{
	...
	case GDMA_EQE_HWC_INIT_DATA:
		...
		case HWC_INIT_DATA_PVF_DEST_RQ_ID:
			hwc->pvf_dest_vrq_id = val;
			break;

		case HWC_INIT_DATA_PVF_DEST_CQ_ID:
			hwc->pvf_dest_vrcq_id = val;
			break;
		}
			  ...
			  const void *req, u32 resp_len, void *resp)
{
	...
	if (gc->is_pvf) {
		dest_vrq = hwc->pvf_dest_vrq_id;
		dest_vrcq = hwc->pvf_dest_vrcq_id;
	}
